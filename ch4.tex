...
We use the algorithm proposed by Thompson and Ken~\cite{thompson1968programming} to construct the FST from the tree of matchers£¬ which is shown in Algorithm \ref{alg:alg_nfa}. Each matcher is a partial Nondeterministic Finite Automaton (NFA), which have one or more dangling arrows, pointing to nothing. We build the whole NFA by connecting the arrows of the partial NFAs. Different operator will have different structures of NFA, which is shown below:
