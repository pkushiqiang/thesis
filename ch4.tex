...
In section C we generally introduced how we use the library of pattern matching over tokens to match the sentences. In this section we will introduce more details of this library, like its advantages and implementation details.
...
Comparing to other state of art machine learning-based information extraction algorithms, our method has such advantages:

\begin{enumerate}
    \item  Easy to implement. According to the pseudo code  in \ref{alg:alg_nfa}, even some undergraduate students without strong machine learning background can implement algorithm in one day.

    \item  No need of data labeling. Supervised machine learning algorithms always need a large labeled training data set, but data labeling is tedious work for users. With our pattern matching algorithm, we avoid the work of data labeling.

    \item  The accuracy of information extraction can increase monotonously as the patterns number increasing. So when we get enough patterns, we can get the result with very high accuracy.
