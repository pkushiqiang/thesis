...
Yao et al.~\cite{lu2013recommender} also presented a hybrid recommender system which integrated content-based and interaction-based relation. In content-based part, relations between job-job, job-job seeker, and job seeker-job seeker can be identified by their similarity of profiles. There two approaches are used to calculate the similarities: For the structured data, like age gender, the weight sum values will be returned; for the unstructured data, like similarity between job and user profile the latent semantic analysis is applied in the system.

In summary, there are some problems exist in previous Job Recommender Systems:
\begin{enumerate}
    \item  Most systems can only process the structured data of r\'esum\'es and job descriptions, but in reality both them are in unstructured formats, like text files or HTML web pages.

    \item  The systems that have information extraction module are designed for recruiters to select applicants, not for job seekers to select jobs.
