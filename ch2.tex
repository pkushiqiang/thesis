...
\end{enumerate}
The system also asked the users upload their resumes, but the resumes are only for facilitating the human judgment. In Malinowski's study, the model latent aspect model is a statistical model, which needs to be trained before it can be applied to recommendation. But training needs labeled data, which are since current quality of result of job searching is poor, it will take a lot of time for user to train model.
...
\begin{table}[ht]
\caption{Features of Resume and Job Description} % title of Table
