...
\centering % used for centering table
\small
\begin{tabular}{  | c | c | c | c | c |c | c |c | p{1cm} | c |  }
 \hline
...
\begin{table}[ht]
\caption{Examples of Matcher } % title of Table
\centering % used for centering table
\begin{tabular}{  | l |  l |  }
 \hline
...
The code is: \\
\texttt{seqMatcher =parser.parse("DE-LEVEL DEGREE ( IN  $\vert$  OF ) DT? MAJOR")}
\\
\\
...
The code is: \\
\texttt{seqMatcher =  UnitMatcher("DE-LEVEL") +  UnitMatcher("DEGREE") +}\\
\hspace{3cm} \texttt{( UnitMatcher("IN") $\vert$ UnitMatcher("OF" ) ) + UnitMatcher("MAJOR")}
\\
\\
...
The pattern is:  "DE-LEVEL DEGREE (IN $\vert$ OF) MAJOR" \\
The code is: \\
\texttt{matcher1 = UnitMatcher("DE-LEVEL") \\
...
\begin{figure}[htbp]
  \centering
  \includegraphics[scale=0.5]{images/test_tokenre2_6.png}
